...
- Mirror env vars (operator only): `DOLT_CREDS_JWK`, `DOLT_CREDS_KEYID` (in all envs; harmless inert when no remote configured), `DOLTHUB_REMOTE_URL` (writer-presence gate), and explicit env-scoped `DOLTHUB_OWNER`. See [`docs/runbooks/dolthub-remote-bootstrap.md`](../../../docs/runbooks/dolthub-remote-bootstrap.md).
