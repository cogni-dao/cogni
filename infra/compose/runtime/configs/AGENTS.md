...
- Infra metrics: `prometheus.exporter.cadvisor` (container memory/CPU/OOM/network/disk) + `prometheus.exporter.unix` (host memory/CPU/filesystem/network, conntrack occupancy, socket state, listen overflow/SYN-cookie/abort-on-memory counters, and repo-owned textfile gauges under `/var/lib/node_exporter/textfile_collector`); requires host mounts `/proc:/host/proc:ro`, `/sys:/host/sys:ro`, `/:/host/root:ro`
