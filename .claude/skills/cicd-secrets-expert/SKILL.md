...
A node-owner sets a secret with only an API key — **proven end-to-end on candidate-a, incl. a non-operator node** (`node-template`)
...
- **preview/prod are NOT provisioned:** prod `createOperatorSecretsPlane` throws → **503** — the `production-node-secrets-writer` OpenBao role isn't wired (`bug.5007`), prod OpenFGA `secrets_manager` bootstrap is unverified, and prod likely lacks the Stakater Reloader controller (`bug.5040`) so a write wouldn't roll the pod. preview is likewise unproven (likely no OpenFGA store → 503 + `preview-node-secrets-writer` + Reloader gaps). Self-serve is candidate-a-only until both are provisioned.
