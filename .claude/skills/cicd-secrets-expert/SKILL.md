...
## Node self-serve secrets — VERIFIED working on ALL THREE envs (2026-06-24)

A node-owner sets a secret with only an API key — **proven end-to-end (write `200` + take-effect) on candidate-a, production, AND preview, incl. a non-operator node** (`node-template`): e.g. `POST cognidao.org/api/v1/nodes/node-template/secrets {env:production}` → `200` (`cogni/production/node-template/X_SELF_SERVE_PROOF` v21), then ESO synced + **Reloader rolled `node-template-node-app`** (take-effect proven; preview proven the same way at v35 after an OpenFGA model re-bootstrap, 2026-06-24)
(`POST <env-host>/api/v1/nodes/<id>/secrets {env, key, value}` → `200 written, version`).
The how-to + the **canonical per-env status table** live in [`docs/guides/secrets-add-new.md`](../../../docs/guides/secrets-add-new.md); the design canon is [`docs/design/node-self-serve-secrets.md`](../../../docs/design/node-self-serve-secrets.md). The non-obvious, hard-won facts:
...
- **prod IS provisioned + e2e-proven (corrected 2026-06-24 — the old "prod 503, candidate-a-only" was STALE):** a **non-operator** node write returns **`200`** and **takes effect** (Reloader rolled the pod) — `production-node-secrets-writer` role + the operator overlay (`OPENBAO_NODE_SECRETS_WRITER_ROLE` via `envFrom` + projected SA token) are wired (#1737), and OpenFGA is **live** on prod (`node-template` → `403 authz_denied`, not 503). Reloader was the lone prod gap (`bug.5040`) — the controller was absent (ESO was present); **installed 2026-06-24** as an Argo Application, so ESO secret updates now roll the annotated pods. A 503 `secrets_plane_config_missing` fires only if `OPENBAO_NODE_SECRETS_WRITER_ROLE` is unset on the pod (a missing OpenBao **role** instead surfaces as **502** at write time); a 503 `authz_unavailable` means the env's OpenFGA model lacks the checked relation. **`preview` is now also proven (fixed 2026-06-24):** it was returning `503 authz_unavailable` because its OpenFGA **model predated the `can_manage_secrets` relation** (#1627) — older actions (deploy-state) returned `403`, secrets `503`. Fix: POST the current `rbac-model.json` to preview's OpenFGA store + repoint `cogni/preview/operator` `OPENFGA_AUTHORIZATION_MODEL_ID` + let Reloader roll the operator. **Root class** ([`task.5049`](../../../docs/spec/ci-cd.md) / work item): normal `skip_infra=true` promotes never re-bootstrap OpenFGA, so new RBAC relations silently never reach an env until a `deploy-infra` run.
...
- **Live on ALL THREE envs (corrected 2026-06-24):** candidate-a, production, AND preview all return `200` on a non-operator self-serve write with take-effect. Preview needed an OpenFGA model re-bootstrap first (its model predated `can_manage_secrets` → `authz_unavailable`); fixed by POSTing the current `rbac-model.json` + repointing the operator. Durable fix (re-bootstrap OpenFGA on every promote, not just `skip_infra=false`): `task.5049`.
