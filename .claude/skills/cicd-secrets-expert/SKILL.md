...
## Node self-serve secrets — VERIFIED working on candidate-a AND production (2026-06-24)

A node-owner sets a secret with only an API key — **proven end-to-end on candidate-a AND production, incl. a non-operator node** (`node-template`): `POST cognidao.org/api/v1/nodes/node-template/secrets {env:production}` → `200` (`cogni/production/node-template/X_SELF_SERVE_PROOF` v21), then ESO synced + **Reloader rolled `node-template-node-app`** (take-effect proven, 2026-06-24)
...
- **prod IS provisioned + e2e-proven (corrected 2026-06-24 — the old "prod 503, candidate-a-only" was STALE):** a **non-operator** node write returns **`200`** and **takes effect** (Reloader rolled the pod) — `production-node-secrets-writer` role + the operator overlay (`OPENBAO_NODE_SECRETS_WRITER_ROLE` via `envFrom` + projected SA token) are wired (#1737), and OpenFGA is **live** on prod (`node-template` → `403 authz_denied`, not 503). Reloader was the lone prod gap (`bug.5040`) — the controller was absent (ESO was present); **installed 2026-06-24** as an Argo Application, so ESO secret updates now roll the annotated pods. The 503 you'll still see is `secrets_plane_config_missing`, which fires only if `OPENBAO_NODE_SECRETS_WRITER_ROLE` is unset on the pod (a missing OpenBao **role** instead surfaces as **502** at write time, not 503). **`preview` is the one still-blocked env:** the preview operator returns **`503 authz_unavailable`** on a correct-env write — its OpenFGA store/model isn't resolvable by the pod, so authz fails closed before any write. Fix preview's OpenFGA bootstrap first; writer-role + Reloader are downstream of that.
...
- **Live on candidate-a AND production (corrected 2026-06-24):** both have a working OpenFGA store (prod write returns `200`; `node-template` → `403`, not 503). **`preview` is the holdout** — its OpenFGA store/model isn't resolvable by the operator pod, so every check there is `authz_unavailable` (503). Full fleet needs OpenFGA fixed on preview.
