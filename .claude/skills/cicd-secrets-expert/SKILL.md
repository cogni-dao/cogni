...
| Rotate VENDOR-MINTED value (OpenAI key, Cherry token) | Operator API route `POST /api/v1/nodes/[id]/secrets`, OpenFGA `can_manage_secrets` — caller holds only an API key       | **SHIPPED + production-live** (#1627 + #1737). Per-env needs the operator writer-identity overlay + `<env>-node-secrets-writer` role; `503 secrets_plane_config_missing` if an env isn't provisioned. `pnpm secrets:set` is the retiring admin fallback. See "Who holds secret-write authority"                                                                     |
...
## Who holds secret-write authority (SHIPPED — agent-without-kube)

The honest answer is now **the node owner, via least-privilege OpenFGA** — not Derek. The value-write path shipped + proven (candidate-a real `200`; **production live** via #1627 + #1737). Three authorities, kept separate:

| Authority                             | Decides            | Who holds it                                                                                                                                                                                |
| ------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Declaration** (shape)               | "node Y may hold K" | GitHub repo merge (catalog PR; **no value in the PR**). Unchanged.                                                                                                                          |
| **Generated value** (`source: agent`) | random bytes for Y | GitHub Actions `materialize-substrate` (`<env>-writer`); prod now provisioned.                                                                                                              |
| **Human/vendor value** (`source: human`) | the byte value  | **The node owner/agent via the operator API + OpenFGA `secrets_manager`** — holds only an API key. The legacy `pnpm secrets:set` + writer-JWT CLI is the **admin fallback, being retired.** |

**Self-serve path (shipped):** `POST /api/v1/nodes/[id]/secrets`, OpenFGA `can_manage_secrets`; the operator pod self-logins with its OWN projected SA token (`audience: cogni-openbao`) and writes — caller holds **only an API key** (no kubeconfig, no vault token). Three gates: OpenFGA fail-closed → gate-2 **denylist** (node owns its namespace; only reserved substrate keys denied — #1721) → OpenBao `_system`/`_shared` deny.

**`secrets_manager` is AUTHORITY, not CUSTODY** (load-bearing — node devs get this wrong and decline the grant):

- **AUTHORITY** = `can_manage_secrets` on `node:<id>` — per-node, the grantee's to hold, only an API key. Sibling of `developer→can_flight` / `production_promoter→can_promote_production`.
- **CUSTODY** = the operator pod's OpenBao writer identity — **never** the grantee's. The operator *mediates*; mediation ≠ the grantee holding operator power.

So a node agent should **request `secrets_manager` on its own node**; "ask an operator to run the CLI for me" re-inserts a privileged human and forfeits self-sovereignty.

**Per-env readiness:** each env needs the operator writer-identity overlay + a provisioned `<env>-node-secrets-writer` role (provision via `provision-env -f mode=substrate-only`, or the routine infra lever once G3 lands). `503 secrets_plane_config_missing` = that env isn't provisioned (ops job, not the node owner's). candidate-a + production live.

Durable how-to: knowledge hub entry `node-self-serve-secrets` + node guide `docs/guides/add-secret.md` (node-template → every fork). Design + remaining work: `docs/design/node-self-serve-secrets.md` + `…-validation-roadmap.md`.
