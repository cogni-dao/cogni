...
A node-owner sets a secret with only an API key — **proven end-to-end on candidate-a, incl. a non-operator node** (`node-template`), **and the WRITE is proven on production** (`POST cognidao.org/api/v1/nodes/operator/secrets {env:production}` → `200`, KV version bumped)
