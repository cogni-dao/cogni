...
4. **Target private GHCR parity.** Akash standard workloads support service-level registry `credentials`, but Cogni's current `ProvisionServiceSpec`/SDL renderer does not render them. Two consumers must not be conflated: test-parent CI uses repo-level `GHCR_DEPLOY_USERNAME` + `GHCR_DEPLOY_TOKEN` to inspect private images; the Akash provider needs a runtime copy held in the private actuator's OpenBao/ESO scope and injected only at SDL-render time—never committed in `XComputeWorkload`, Git, or a test-repo manifest. GitHub Packages currently requires a classic PAT with `read:packages`; use a dedicated test bot and grant it only package read access. A missing repo secret and a missing SDL capability are separate failures. Until the latter lands, making one disposable package public is an explicit temporary E2E exception, not a node-birth invariant and not a reason to make `node-template` public.
