...
2. **Set all required secrets:**
   ...
   - **Service secrets:** Fresh generation per environment
     ...
     - **OAuth providers (optional — provider silently skipped if missing):**
       ...
       - `DOLTHUB_API_TOKEN` (from dolthub.com → Settings → API Tokens, scope `api_read_write`; **v0 push job uses this** — app-level, one-way mirror prod → `cogni-dao/knowledge-<node>`. **One token on the OPERATOR owns push for all nodes.** Per task.5069)
