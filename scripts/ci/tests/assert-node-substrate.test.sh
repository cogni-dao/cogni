...
TMPROOT=$(mktemp -d -t assert-node-substrate.XXXXXX)
...
BASE_ENV=(
  ...
  CHECK_DNS=false
  ASSERT_NODE_SUBSTRATE_SSH_BIN="$FAKEBIN/ssh"
  ASSERT_NODE_SUBSTRATE_REMOTE_ROOT="$REMOTE_ROOT"
  ASSERT_NODE_SUBSTRATE_APP_WAIT_ATTEMPTS=1
  ASSERT_NODE_SUBSTRATE_APP_WAIT_SLEEP_SECONDS=0
  FAKE_REMOTE_PATH="$FAKEBIN"
)

env "${BASE_ENV[@]}" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/success.out"
...
env "${BASE_ENV[@]}" CHECK_DNS=true \
  ...
  CF_CURL="$FAKEBIN/cf-curl" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/scoped-dns.out"
grep -q "Node substrate ready for canary" "$TMPROOT/scoped-dns.out"

if env NODE=canary DEPLOY_ENVIRONMENT=candidate-a VM_HOST="" DOMAIN=test.cognidao.org \
  APP_SOURCE_DIR=. COGNI_CATALOG_ROOT=infra/catalog CHECK_DNS=false \
  ASSERT_NODE_SUBSTRATE_SSH_BIN="$FAKEBIN/ssh" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-vm.out" 2>&1; then
  echo "expected missing VM_HOST to fail" >&2
  exit 1
fi
grep -q "VM_HOST is required" "$TMPROOT/missing-vm.out"

if env "${BASE_ENV[@]}" APP_SOURCE_DIR="$TMPROOT/no-catalog" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-catalog.out" 2>&1; then
...
if env "${BASE_ENV[@]}" APP_SOURCE_DIR="$TMPROOT/no-overlay" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-overlay.out" 2>&1; then
...
if env "${BASE_ENV[@]}" APP_SOURCE_DIR="$TMPROOT/no-appset" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-appset-file.out" 2>&1; then
  echo "expected missing appset file to fail" >&2
  exit 1
fi
grep -q "missing per-node AppSet file" "$TMPROOT/missing-appset-file.out"

if env "${BASE_ENV[@]}" FAKE_MISSING_NAMESPACE=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-namespace.out" 2>&1; then
  echo "expected missing namespace to fail" >&2
  exit 1
fi
grep -q "namespace missing" "$TMPROOT/missing-namespace.out"

if env "${BASE_ENV[@]}" FAKE_MISSING_APPSET=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-appset.out" 2>&1; then
  echo "expected missing remote AppSet to fail" >&2
  exit 1
fi
grep -q "ApplicationSet missing" "$TMPROOT/missing-appset.out"

if env "${BASE_ENV[@]}" FAKE_MISSING_APPLICATION=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-application.out" 2>&1; then
  echo "expected missing remote Application to fail" >&2
  exit 1
fi
grep -q "Argo Application missing" "$TMPROOT/missing-application.out"

if env "${BASE_ENV[@]}" FAKE_MISSING_SECRET=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-secret.out" 2>&1; then
  echo "expected missing secret to fail" >&2
  exit 1
fi
grep -q "runtime Secret missing" "$TMPROOT/missing-secret.out"

if env "${BASE_ENV[@]}" FAKE_MISSING_DB=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-db.out" 2>&1; then
...
if env "${BASE_ENV[@]}" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-edge-env.out" 2>&1; then
...
if env "${BASE_ENV[@]}" FAKE_MISSING_LIVE_CADDY_ROUTE=1 bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-live-caddy.out" 2>&1; then
  echo "expected missing live Caddy route to fail" >&2
  exit 1
fi
grep -q "live Caddy config missing" "$TMPROOT/missing-live-caddy.out"

if env NODE=canary DEPLOY_ENVIRONMENT=candidate-a VM_HOST=192.0.2.10 DOMAIN=test.cognidao.org \
  APP_SOURCE_DIR=. COGNI_CATALOG_ROOT=infra/catalog CHECK_DNS=true \
  ASSERT_NODE_SUBSTRATE_SSH_BIN="$FAKEBIN/ssh" ASSERT_NODE_SUBSTRATE_REMOTE_ROOT="$REMOTE_ROOT" \
  FAKE_REMOTE_PATH="$FAKEBIN" bash scripts/ci/assert-node-substrate.sh >"$TMPROOT/missing-dns.out" 2>&1; then
  echo "expected missing DNS inputs to fail" >&2
  exit 1
fi
grep -q "CLOUDFLARE_API_TOKEN required" "$TMPROOT/missing-dns.out"

echo "PASS: assert-node-substrate.test.sh"
