...
put_secret oss APP_DB_PASSWORD pernode-app-pw-sentinel
put_secret oss APP_DB_SERVICE_PASSWORD pernode-svc-pw-sentinel
...
# from OpenBao (the put_secret oss ... above), read via the db-reader token —
...
env \
  ...
  bash scripts/ci/reconcile-node-substrate.sh candidate-a oss > "$TMPROOT/out.txt"

grep -q "substrate ready inputs reconciled for oss" "$TMPROOT/out.txt"
...
after_keys="$(cd "$BAO_ROOT/cogni/candidate-a/oss" && printf '%s\n' * | sort | paste -sd, -)"
if [ "$after_keys" != "APP_DB_PASSWORD,APP_DB_SERVICE_PASSWORD" ]; then
  echo "reconcile must perform ZERO OpenBao writes; oss bank changed to: $after_keys" >&2
  exit 1
fi
grep -q '^OSS_DOMAIN=oss-test.cognidao.org$' "$REMOTE_ROOT/opt/cogni-template-edge/.env"
grep -q 'COGNI_NODE_DBS=cogni_operator,cogni_oss' "$REMOTE_ROOT/opt/cogni-template-runtime/.env"
...
grep -q -- '-e COGNI_NODE_DBS=cogni_oss' "$REMOTE_ROOT/docker.log"
...
env \
  ...
  FAKE_REMOTE_PATH="$FAKEBIN" \
  FAKE_BAO_ROOT="$BAO_ROOT" \
  bash scripts/ci/reconcile-node-substrate.sh candidate-a oss > "$TMPROOT/relative-catalog-root.out"
grep -q "substrate ready inputs reconciled for oss" "$TMPROOT/relative-catalog-root.out"
