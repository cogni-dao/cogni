...
#   2. External artifact rows do not require repo-spec identity during parent rendering.
...
for node in "${NODE_TARGETS[@]}"; do
  if is_external_artifact_target "$node"; then
    case ",$endpoints," in
      *",$node="*) fail "COGNI_NODE_ENDPOINTS includes external artifact node $node before metadata projection exists" ;;
      *) pass "$node external artifact endpoint skipped" ;;
    esac
    continue
  fi
...
echo "[2/3] external artifact catalog nodes are skipped during parent endpoint rendering"
TMP_TREE="$(mktemp -d)"
...
fixture_endpoints="$(COGNI_CATALOG_ROOT="$TMP_CATALOG" bash scripts/ci/render-scheduler-worker-endpoints.sh)" \
  || fail "render failed for an external artifact catalog node without nodes/ay/.cogni/repo-spec.yaml"
case ",$fixture_endpoints," in
  *,ay=*) fail "fixture endpoints include external artifact slug ay" ;;
  *) pass "external artifact slug ay omitted" ;;
esac
case "$fixture_endpoints" in
...
fixture_billing_endpoints="$(COGNI_CATALOG_ROOT="$TMP_CATALOG" bash -c 'source scripts/ci/lib/image-tags.sh && node_billing_endpoint_csv host.docker.internal')" \
  || fail "billing endpoint render failed for an external artifact catalog node without nodes/ay/.cogni/repo-spec.yaml"
case ",$fixture_billing_endpoints," in
  *,ay=*) fail "billing endpoints include external artifact slug ay before metadata projection exists" ;;
  *) pass "billing endpoint skips external artifact slug ay" ;;
esac
case "$fixture_billing_endpoints" in
