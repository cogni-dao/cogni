...
#
# Output layout: per-(env, node) AppSet files live in infra/k8s/argocd/appsets/
#   alongside a GENERATED appsets/kustomization.yaml that lists them. That dir is
#   reconciled with prune by the cogni-appsets app-of-apps, so removing a node
...
# The per-(env, node) AppSets live in their own dir, reconciled with prune by the
# `cogni-appsets` app-of-apps (cogni-appsets-application.yaml). appsets/kustomization.yaml
# is GENERATED WHOLESALE here (not a spliced block in the bootstrap kustomization),
# so a catalog-removed node's AppSet vanishes from git and Argo auto-prunes it.
APPSETS_DIR="$ARGOCD_DIR/appsets"
KUSTOMIZATION="$APPSETS_DIR/kustomization.yaml"
...
# The WHOLE appsets/kustomization.yaml: a self-contained kustomization (header +
# apiVersion + kind + namespace + resources) listing the local per-(env, node)
# AppSet files env-major (candidate-a, preview, production), node-sorted (LC_ALL=C).
# Sourced as a dir by the `cogni-appsets` app-of-apps; no bootstrap-splice sentinels.
...
write_kustomization() {
  render_kustomization > "$KUSTOMIZATION"
}

write() {
  local env node count=0 expected="" pruned=0 committed base
  for env in "${ENVS[@]}"; do
    for node in $(deployable_nodes_for_env "$env"); do
  ...
  # Prune AppSets a node no longer claims (removed from its `envs:` set, or the row
  # left the catalog) so `pnpm gen:node-appset` is self-healing — and the cogni-appsets
  # app-of-apps then prunes the live AppSet once the file leaves git.
  for committed in "$APPSETS_DIR"/*-applicationset.yaml; do
  ...
  write_kustomization
  echo "Wrote $count per-node ApplicationSet files (pruned $pruned stale) + appsets kustomization."
}
...
check() {
  ...
  # All files under appsets/ are renderer-owned — candidate-b and other manually
  # managed envs keep their own appset shape in the parent dir, out of scope here.
  local base
  for committed in "$APPSETS_DIR"/*-applicationset.yaml; do
    [ -e "$committed" ] || continue
    base="$(basename "$committed")"
  ...
  if ! diff -u "$KUSTOMIZATION" <(render_kustomization) >/dev/null; then
    echo "[ERROR] $KUSTOMIZATION is out of sync with the catalog:" >&2
    diff -u "$KUSTOMIZATION" <(render_kustomization) >&2 || true
    stale=1
  fi
  if [ "$stale" -ne 0 ]; then
  ...
  echo "per-node ApplicationSet files + appsets kustomization are in sync with the catalog."
}
