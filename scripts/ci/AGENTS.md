...
- `promote-build-payload.sh` translates a resolved image payload into overlay mutations via `promote-k8s-image.sh`, writes `promoted_apps=<csv>` to `$GITHUB_OUTPUT` incrementally after each successful promotion (trap EXIT guarantees the exit-time write even on abort — bug.0328), and merges per-app `source_sha` into `.promote-state/source-sha-by-app.json`. `NODES=<csv>` scopes catalog nodes and also includes payload entries whose `overlay_target` matches a scoped node, allowing remote-source Shape B sidecars to promote with their owning app overlay. Map-write failures are `::warning::` annotations; total map-write failure exits non-zero so provenance decay cannot silently persist.
