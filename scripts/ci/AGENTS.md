...
- `lib/image-tags.sh` is a thin shim that populates `ALL_TARGETS` / `NODE_TARGETS` and resolves image tags, node ports, node IDs, DB names, and endpoint CSVs by reading `infra/catalog/*.yaml` at source time via the pre-installed `yq`. Repo-root-relative resolution; override via `COGNI_CATALOG_ROOT` for fixtures.
