...
# PR file list matches both its signed hash and the env-manager path boundary.
...
if ! jq -e \
  ...
  '.user.login == $login
   ...
   and (.head.ref | test("^cogni-operator/node-(env|placement)-"))' \
...
[[ "$action" =~ ^(add|remove|place-k3s|place-akash)$ ]] || reject_claim invalid-action
...
case "$action" in
  ...
  remove)
    ...
    ;;
  place-k3s|place-akash)
    [[ "$head_ref" == "cogni-operator/node-placement-$node-$env_name" ]] || reject_claim invalid-branch
    [[ "$subject" == "feat(node): place $node $env_name on ${action#place-}" ]] || reject_claim invalid-subject
    ;;
esac
...
while IFS= read -r changed_file; do
  case "$changed_file" in
    ...
    "infra/k8s/overlays/$env_name/$node/external-secret.yaml")
      [[ "$action" == "add" || "$action" == "remove" ]] || reject_claim placement-crossed-path-boundary
      ;;
    infra/k8s/argocd/appsets/candidate-a/"$env_name-$node-applicationset.yaml"|\
    infra/k8s/argocd/appsets/preview/"$env_name-$node-applicationset.yaml"|\
    infra/k8s/argocd/appsets/production/"$env_name-$node-applicationset.yaml")
      [[ "$action" == "add" || "$action" == "remove" ]] || reject_claim placement-crossed-path-boundary
      appset_seen=true
      ;;
...
if [[ "$action" == "add" || "$action" == "remove" ]]; then
  ...
  # A placement normalization can be catalog-only when the scheduler route was
  # already correct; the deterministic scheduler drift check proves that state.
  [[ "$file_count" -le 2 ]] || reject_claim incomplete-placement-change
fi
