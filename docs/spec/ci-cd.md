...
**ONE artifact identity (`SOURCE_SHA_IS_DEPLOY_IDENTITY`).** Every deployable — node OR the operator app — is published and resolved as `<image>:sha-<sourceSha>`. The legacy `pr-{N}-{sha}` / `mq-{N}-{sha}` / `preview-{sha}` namespaces are **purged** (#1800): `flight-preview.yml` resolves the merge's `sha-<mainSha>` directly with **no re-tag**, and preview/prod promotion carries that exact digest forward. `pr-build.yml` self-builds `sha-<mainSha>` on `push:main` (so EVERY merge — queue, direct, admin, or operator `/vcs/merge` — yields a deployable image), in addition to `merge_group` (the queue's `manifest` check) and `pull_request`. Maintenance commits on `main` that carry no deployable change should not build/dispatch; `pr-build.yml` + `flight-preview.yml` skip CI-owned prefixes such as `chore(deps): argocd-image-updater` and `chore(preview):`. Keying is **message-prefix**, not author identity (`github-actions[bot]` is shared).
...
- Production promotion: `POST /api/v1/deploy/promote {nodeId, env:"production", sourceSha}` (operator-API, `can_promote_production`-gated) — or a human `workflow_dispatch` of `promote-and-deploy.yml` — carries the preview-proven digest forward and copies the source map. In-repo nodes (operator) resolve `<image>:sha-<sourceSha>` directly (the `preview-{sha}` lookup is purged, #1800). PROMOTION_RUNS_AS_THE_OPERATOR; ZERO commits to `main`.
