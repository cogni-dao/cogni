...
Application names stay `<env>-<node>` across **both** transitions — and that invariance is exactly what lets bug.0378's per-node AppSets **adopt** the pre-existing Applications during cutover (same generated name, new owner). task.0372 flipped each Application's `targetRevision` from the whole-slot `deploy/<env>` to the per-node `deploy/<env>-<node>`; bug.0378 changed only the *owning AppSet object's* name (`cogni-<env>` → `cogni-<env>-<node>`), never the generated Application name.
