...
| **Developer-observability** | dev → operator **proxies** node-scoped (dev holds no token) | Grafana/Loki read, PostHog read, read-only DB                                | **this plane** — node-scoped reads behind the `developer` gate                                                                     |
...
4. **`app_<node>_readonly` role** — trivial loop add; per-node DB read via a scoped DSN.
