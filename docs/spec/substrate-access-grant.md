...
| **`nodeId` on Langfuse traces** (the real blocker) | 🔴     | trace carries `tags:[providerId,graphId]` + metadata with **no node** (`observability-executor.decorator.ts`)                                | stamp `nodeId` as a trace tag + metadata field (decorator already binds `billingAccountId`; inject `getNodeId()` the same way) — **nothing isolates without it** (the MVP task) |
| Langfuse node-pinned **proxy**                     | 🔴     | `LANGFUSE_*` `shared:true`, one project (`infra/secrets-catalog.yaml`)                                                                       | `GET /nodes/{id}/observability/traces` — operator runs the dev's trace-list AND-ed with `nodeId=<id>` via the operator-held key; dev holds nothing (after the tag)              |
