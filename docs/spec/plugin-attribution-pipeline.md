...
`cogni-v0.0` is the active profile. Its `main-merge-selection.v0` policy treats a PR merged to `main` **as** the contribution (reviews on those PRs included for visibility; bots excluded). It replaced an earlier `promotion-selection.v0`, which modelled a staging→main promotion flow and treated a direct-to-`main` PR as a "release PR" (reference data, **excluded**) — yielding zero claimants on a repo that merges directly to `main` after canary/staging was retired. The repointing was a one-time pre-release amendment (see `PROFILE_IMMUTABLE_PUBLISH_NEW`): v0.0 was still unused by any node and had produced zero claimants, so publishing a parallel `cogni-v0.1` purely to flip the schedule input would have been ceremony. `promotion-selection.v0` stays registered as an available policy. Going forward, the documented upgrade path is publish-new + bump `attribution_pipeline` in `repo-spec.yaml`.
