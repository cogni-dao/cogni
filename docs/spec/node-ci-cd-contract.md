...
- **P0 — Parent CI must treat a submodule gitlink as operator-domain-only, never a build target.** The pin-PR splices `<slug>` into `.github/workflows/ci.yaml` (scope-filter) and adds a `nodes/<slug>` gitlink, so `detect-affected.sh` schedules a `build (<slug>)` leg (fails — a bare gitlink has no app tree) and the `nodes/*`↔filter mirror meta-test breaks (the filter now lists `<slug>` with no inline `nodes/<slug>/`). `SUBMODULE_GITLINK_IS_OPERATOR_PIN` (#1495) fixes domain _classification_ but not _affected/build_ detection. Fix: `detect-affected.sh` + the mirror meta-test recognize a gitlink (`.gitmodules` / `160000` mode) and exclude it from build targets — a pinned node is built by its own repo's CI, never the parent. **Hard stop: the pin-PR cannot merge until this lands.** (Observed on `cogni-test-org/cogni-monorepo#1`.)
