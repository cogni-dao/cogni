...
- **`ONE_PROMOTION_PRIMITIVE`**: every rung — candidate-a, preview, production — dispatches `promote-and-deploy` directly by digest. No rung routes through a code-branch PR (task.5022 retired the last one: preview's catalog-pin PR stalled open forever, since a catalog-only PR earns no required merge_group checks and a bot catalog commit carries no `(#NNN)` suffix for `flight-preview.yml` to resolve). Preview is **source-addressed** like `candidate-flight`: the node image sha rides the dispatch as `node_source_sha`, the workflow's remote-source digest resolver prefers that input over the catalog `source_sha` read (production omits it ⇒ catalog read, unchanged), and the pin is recorded on `deploy/preview` (`update-source-sha-map.sh`). Preview writes **ZERO commits to `main`**. No env rebuilds, no second dispatch path, no PR-tag substitution.
