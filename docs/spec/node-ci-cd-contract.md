...
**Today's gap (operator-hosted envs).** The invariant covers secret _shape_, but the secret _value_ has no self-serve node-owner path. The `<env>-writer` role is minted only via that env's cluster custody (kubeconfig → `openbao-operator` SA token), so on an operator-hosted env the value-set is operator-custody-gated — a node-owner cannot set or deploy a secret value without that custody. "A node-dev granted the env-writer role" above is therefore aspirational on operator-hosted envs. The forward fix is an operator-mediated, OpenFGA-authorized secret API: a `developer`-granted node-owner sets values through the operator (which holds the machine writer credential), never holding cluster custody itself — the same grant model the authz substrate already establishes.
