...
1. **PATH_CONVENTION_PER_SERVICE_PER_ENV.** Every secret lives at `cogni/<env>/<service>` in OpenBao KV v2, with the secret name as a key at that path. `<env>` ∈ {`candidate-a`, `preview`, `production`}; `<service>` is the catalog name (`node-template`, `scheduler-worker`, `akash-tx-actuator`, …). One path per (service, env). Multiple keys per path. **`<service>` is a blast-radius boundary, not a naming convenience:** because Invariant 2 extracts an entire path into one k8s Secret and Invariant 3 hands that whole Secret to a pod, anything sharing a path shares a compromise. A credential whose blast radius must be smaller than its owning node's gets its own `<service>` — e.g. `akash-tx-actuator` holds the Akash wallet credential so that a compromise of the public operator app, which consumes all of `cogni/<env>/operator`, cannot reach it.
   ...
   **OPEN (story.5016 seam 5): `<env>` names the ENVIRONMENT; which OpenBao custodies that path is now a separate question.** One OpenBao runs per VM, so until Akash compute arrived the two read as the same thing and per-cluster custody was assumed without ever being stated. The Akash north star ([`knowledge:akash-actuator-wallet-cutover`](https://cognidao.org/knowledge/akash-actuator-wallet-cutover)) breaks the tie: one production Console account pays for every node in every environment, which forces one active writer, which — because every workload env value is a provider-http placeholder resolved **in the reconciling cluster** — puts every environment's _compute_ desired state in the production cluster. So a pre-prod Akash workload's values must be readable FROM the production cluster.

   **Neither way of achieving that exists yet.** Both were probed empirically; record them before proposing a third:

   | approach                                             | blocked by                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
   | ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | Production's OpenBao custodies the pre-prod bucket   | **No write path.** Writes are env-local by contract: `POST /nodes/<id>/secrets` validates the requested env against the operator's own `DEPLOY_ENVIRONMENT` and returns `409 wrong_operator_env` otherwise, and `<env>-node-secrets-writer` is scoped to `cogni/data/<env>/*` in its own env's OpenBao. Nothing can write `cogni/candidate-a/*` into production's OpenBao except the root token (Invariant 13 forbids that as a standing path).                                                                                                            |
   | Production reads the pre-prod env's OpenBao directly | **No read path.** OpenBao is `serviceType: ClusterIP` with `tls_disable = 1` and "in-cluster only (no Ingress in v0)" (`infra/k8s/argocd/openbao/values.yaml`); its Kubernetes auth is configured `kubernetes_host=https://kubernetes.default.svc:443`, so it validates SA JWTs against **its own** cluster's TokenReview and cannot authenticate a foreign cluster's ESO. Reaching it cross-cluster needs a new network entry point, TLS that is not shipped (task.5054), and a new auth mount — which Invariant 9 does not permit as an ad-hoc addition. |

   Two things are settled regardless of how that resolves:
   - **Custody flows DOWN-TRUST, one direction only.** No production credential in a candidate-a or preview cluster, ever. A production-custody path (`cogni/production/*`) is always read in-cluster; that is asserted by `tests/ci-invariants/akash-lane-compute-env-custody.spec.ts`.
   - **Exactly one canonical copy per path.** Whichever OpenBao ends up serving `cogni/<env>/<slug>` for compute, the same logical path must not be populated in two OpenBaos with no reconciler — that is a silent-drift hazard, not a redundancy.
