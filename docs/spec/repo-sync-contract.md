...
| **3 — Node identity / presentation** | the node's _face_: homepage / landing UI (`app/src/app/(public)/**`), home feature surface (`app/src/features/home/**`), branding/theme/assets, `.cogni/repo-spec.yaml`, persona — declared in `.cogni/sync-manifest.yaml#node_local` | **NEVER synced.** Left as the fork's own version (never overlaid)                                              | n/a — opens no PR                                                           |
