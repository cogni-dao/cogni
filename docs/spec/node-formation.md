...
2. **ARAGON_MINTED_TOKEN**: Use Aragon's GovernanceERC20 minted during DAO creation. No custom NonTransferableVotes deployment. Tokens are transferable.
   ...
   Distribution readiness is non-linear with formation. A newly formed node and an already-deployed DAO both publish the same repo-spec lifecycle: `governance.token_contract` identifies the Aragon `GovernanceERC20` when known, while `distributions.status: pending_activation` remains separate from `payments.status`. Distribution activation is a later repo-spec update: the V0 pathway verifies the DAO contract itself as the DAO-controlled emissions holder, verifies it has minted token inventory, opens a one-file PR against the node's own repo, records `governance.emissions_holder`, pins `distributions.claim_contract_pattern: uniswap.merkle-distributor.v1`, and flips `distributions.status: active`. Existing DAO nodes do not replay formation; they run this activation against their existing repo-spec.
