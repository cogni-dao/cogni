...
**DELETE_IS_CLEAN** — Dolt is git-versioned, so a `DELETE` preserves the row's content, every commit, and the contributor chain in history (the log _is_ the tombstone — audit + attribution survive). So dead/retired knowledge (no successor) is **deleted** from the live table, not soft-flagged — recall and the cognition bundle never surface corpses. **Supersession is REFINE in place** (update the same row id; the old version stays in history), not write-new-then-flag-old. Deletion is DAG-safe: refuse if a live row cites the target (repoint/remove those edges or refine instead); the entry's own outbound edges cascade. (The `status` column + its `deprecated` value remain only for the EDO temporal hypothesis lifecycle, not as a generic knowledge soft-delete.)
...
| DELETE_IS_CLEAN                          | Dead/retired knowledge is DELETEd from the live table (Dolt history preserves content + commits + attribution). Supersession is REFINE-in-place, not deprecate-and-replace. Deletion refuses if inbound-cited; cascades own outbound edges. (`status='deprecated'` retained only for the EDO hypothesis lifecycle.)                                                                                                                                                                                                                        |
