...
| **P1** — EDO Foundation (`proj.edo-foundation`)                             | Closes the data → hypothesis → action → data loop. Schema extension (`evaluate_at` + `resolution_strategy` + new `entry_type` and citation type values) · `EdoResolverPort` + 1-hop `recomputeConfidence` (pure-from-citations) · three atomic agent tools (`core__edo_{hypothesize,decide,record_outcome}`) · `resolveDueHypotheses` cron in `scheduler-worker` · brain prompt teaches the loop · `/knowledge?mode=chains` DAG view. Designed in § The Hypothesis Loop above; delivered as Crawl/Walk/Run tiers under one project. | **Crawl tier in flight (this PR / proj.edo-foundation).** Walk + Run tiers follow.                      |
