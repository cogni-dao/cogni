...
**Boundaries.** The bundle is **authed and index-only** — any principal (cookie-session human OR `cogni_ag_sk_v1_` agent bearer) gets skill/domain pointers (title, use-when, recall path), never full entry bodies. It sits behind the same gate as the read routes (`KNOWLEDGE_READ_REQUIRES_PRINCIPAL`), so the index does not diverge from the read surface; `GET /api/v1/agent/register` stays the one public bootstrap seam (register → key → cognition), and the SessionStart loader passes `COGNI_API_KEY` as the bearer. Root `AGENTS.md` drops to a thin bootstrap pointer at the bundle; node-scoped and subdir `AGENTS.md` files remain (closest-file-wins still holds for code-local rules). This is reversible: revert the PR and `AGENTS.md` carries the full orientation again.
