...
| Orientation entry  | the `<slug>-agent-orientation` hub entry, rendered IN FULL                 | knowledge hub |
...
The bundle's stance is **constitution + map**: the code-owned invariants say how every agent must behave; the repo-spec mission says why this node exists; the orientation entry is the current-node map an agent needs to start (where to edit, what not to run, what can break prod/candidate, what to recall next). Skills and domains stay **index-first** (pointers only), but the current-node orientation entry is rendered **in full** — the bootstrap IS the operating map, with no second recall hop before an agent has its bearings.
