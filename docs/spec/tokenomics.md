...
- Before Walk settlement can go live, repo-spec must carry `distributions.status: active`, `governance.token_contract`, `governance.emissions_holder`, and the selected OSS claim pattern. Activation is a git-governed node repo-spec PR, not a formation-only checkbox: new nodes and existing DAO nodes both use the same update flow once DAO-controlled minted inventory exists. V0 uses one concrete path: the emissions holder is the DAO contract address, and activation verifies the DAO contract holds a nonzero GovernanceERC20 balance before writing `distributions.status: active`.
