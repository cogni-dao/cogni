...
6. **RPC_IN_ENVIRONMENT**: `EVM_RPC_URL` varies per deployment, never committed. It is a **provisioned per-env `_shared` substrate** (`node declares shape; operator wires environment`, [node-baas-architecture.md](./node-baas-architecture.md)). Within the managed fleet it is **inherited**, not human-supplied per node: the **operator owns one value per env** at `cogni/<env>/_shared/EVM_RPC_URL`, and every spawned node inherits it via `secret-materialize.sh`'s ancestor scan. The lone human action is the operator (or a **standalone sovereign fork**, which has no operator ancestor to inherit from) seeding it **once** — `source: human` denotes byte-origin (a vendor RPC URL), not a per-node paste. `EVM_RPC_URL` is `appliesTo: all-nodes, shared: true, required: true, source: human`; `secret-materialize.sh` fails-fast when a `required` **`source: human`** key resolves empty (repair the bank, never the node) so a missing endpoint surfaces loudly at provision/flight instead of silently no-op'ing outbound web3 at runtime (bug.5087). (Scoped to `source: human` — an empty agent/composed key is a generator gap, a different class.) Follow-ups: capability-gate requiredness (chain-only for on-chain/payments nodes); and consider `inheritFrom: operator` (as `OPENROUTER_API_KEY` already uses) to make fleet-inheritance explicit rather than a blind ancestor scan.
