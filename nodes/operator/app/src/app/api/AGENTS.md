...
- **Routes (if any):**
  ...
  - `/api/v1/nodes/[id]/provision-wallet` [POST] - server-side Privy wallet provisioning, advances dao_formed → wallet_ready
  - `/api/v1/nodes/[id]/publish` [POST] - opens a PR with `.cogni/repo-spec.yaml` on the target repo, advances payments_ready → active
