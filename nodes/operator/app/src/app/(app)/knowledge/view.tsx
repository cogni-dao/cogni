...
import { GitBranch, GitMerge, Library, Plus, Settings2, Tags } from "lucide-react";
...
function isMode(v: string | null): v is ViewMode {
  return (
    v === "browse" || v === "domains" || v === "inbox" || v === "chains"
  );
}
  ...
  return (
    <>
      ...
      {error ? (
        ...
        mode === "chains" ? <ChainsEmptyState /> : <BrowseEmptyState />
      ) : (
        <DataGrid
          table={table}
  ...
  return (
    <div className="flex flex-col items-center gap-2 rounded-lg border border-border/60 bg-muted/20 px-6 py-16 text-center">
      ...
      <p className="max-w-md text-muted-foreground text-xs leading-relaxed">
        ...
        <code className="font-mono">/api/v1/edo/hypothesize</code> (or the
        agent tool <code className="font-mono">core__edo_hypothesize</code>)
