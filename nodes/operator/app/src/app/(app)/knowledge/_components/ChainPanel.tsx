...
export function ChainPanel({ rootId }: ChainPanelProps) {
  ...
  const byDepth = useMemo(() => {
    if (!chainQuery.data) return [] as Array<{ depth: number; nodes: ChainNodeDto[] }>;
  ...
  if (chainQuery.isLoading) {
    return (
      <p className="text-muted-foreground text-xs">Walking chain…</p>
    );
  }
  if (chainQuery.error) {
    return (
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {byDepth.length === 0 || (byDepth.length === 1 && byDepth[0]!.nodes.length === 1) ? (
...
function inferEntryType(id: string): string | null {
  if (id.includes("hypothesize") || id.includes("hypothesis")) return "hypothesis";
