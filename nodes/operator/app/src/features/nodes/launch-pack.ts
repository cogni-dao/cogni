...
export function buildNodeLaunchPack(
  ...
  const prompt = [
    ...
    "5. Checkpoint with the human: give a brief status, progress, and next-steps summary; then guide them through the production browser-session flight auth setup at https://github.com/Cogni-DAO/cogni/blob/main/docs/guides/browser-session-flight-auth.md so the operator API can approve this AI developer and flight this creator-owned node.",
    ...
    "9. Present the human scorecard only after flight, /version, and agent-first validation are green.",
    "",
