...
export function buildNodeLaunchPack(
  ...
  const prompt = [
    ...
    "5. Treat secret/substrate readiness as a three-env birth invariant: candidate-a, preview, and production must be able to materialize `<slug>-env-secrets` from OpenBao via ESO. The sanctioned readiness lane preserves existing values, generates only missing source: agent app material, updates DB/edge substrate, and never uses legacy `<slug>-node-app-secrets`.",
    "6. Checkpoint with the human: give a brief status, progress, and next-steps summary; then follow the Cogni knowledge block above for agent registration, human node-developer approval, bearer-token nodeRef flight, and cleanup/revoke steps.",
    "7. Request candidate-a flight through the operator API only after the child image tag exists and the parent pin agrees.",
