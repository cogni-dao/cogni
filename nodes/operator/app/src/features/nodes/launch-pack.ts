...
export function buildNodeLaunchPack(
  ...
  const prompt = [
    ...
    "",
    "You are the AI developer taking this node from spawned scaffold to first deployed customization. You start with ZERO privileged GitHub access — you request developer access FOR YOUR OWN GitHub account, and the node owner approving that grant ONCE both gives your GitHub account branch-push on the node repo AND authorizes every privileged step (CI, build, flight, merge, promote) through the operator API on your behalf. That single Approve click is the only human step in the whole path.",
    ...
    "",
    `Node-specific guardrails for THIS node: fire the developer-access request immediately — POST ${OPERATOR_API_ROOT}/api/v1/nodes/${input.nodeId}/access-requests with your bearer (the full request → owner-Approve → branch-push contribution flow lives in \`cicd-e2e-required-sequence\` — follow that guide, do not reinvent it here) so the owner's Approve surfaces early and runs in parallel with your customization; the owner approves once in the node UI and your bearer can use the grant but never self-approve; do not push to \`main\` or hand-edit the operator gitlink; keep the repo-spec knowledge.remote (do not add a DOLTHUB_REMOTE_URL override); present the scorecard only after flight + /version + agent-first validation are green. If a step is blocked, report the exact blocked scorecard row instead of inventing a privileged manual step.`,
  ].join("\n");
