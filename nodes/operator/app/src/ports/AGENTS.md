...
- AkashTxMigrationRequirement + AkashTxMigrationPort (MIGRATION_BEFORE_TRANSACTION: every mutating actuator call states its migration precondition, and `RequireBeforeTransaction` is proven complete before anything is spent — bug.5116 ordering, bug.5140 enforcement)
