...
/**
 * Remove a node's `<slug>=…` + `<node_id>=…` aliases from the scheduler-worker configmap's
 * `COGNI_NODE_ENDPOINTS` CSV — the inverse of {@link insertSchedulerEndpoint}. Used on full node
 * decommission (story.5020); byte-identical to `pnpm gen:scheduler-worker-endpoints` once the node
 * leaves the catalog. Idempotent: a configmap that does not list the slug is returned unchanged.
 */
export function removeSchedulerEndpoint(
  ...
  const kept = pairs.filter((p) => p.slug !== slug);
  if (kept.length === pairs.length) {
    return currentConfigmap;
  }

  const csvOut = kept.map((p) => p.text).join(",");
  const lineOut = `${indent}${ENDPOINTS_KEY}: "${csvOut}"`;
  return currentConfigmap.replace(line, lineOut);
}
