 ...
 * Purpose: node-template's Doltgres knowledge schema. Re-exports the syntropy seed
 *   bundle from @cogni/knowledge-base and owns node-template-specific contribution
 ...
 *   - SCHEMA_GENERIC_CONTENT_SPECIFIC: node-template-specific content lives in rows (domain + tags), not columns. Add companion tables here only for genuinely new entities.
 *   - Dialect separation: this package is NOT globbed by nodes/node-template/drizzle.config.ts (which targets Postgres); only by nodes/node-template/drizzle.doltgres.config.ts.
 * Side-effects: none
 * Links: docs/spec/knowledge-data-plane.md, docs/spec/knowledge-syntropy.md, work/items/task.5077.node-template-doltgres-substrate.md
 * @public
 */
