# hello-world-doltgres-schema · AGENTS.md
...
- **Owners:** @derekg1729
- **Status:** stable
- **Package:** `@cogni/hello-world-doltgres-schema`

## Purpose

Pure re-exports of the Drizzle table definitions from `@cogni/knowledge-base`, scoped to the hello-world `knowledge_node_template` database. Owned by and namespaced to the hello-world node. Mirrors `@cogni/operator-doltgres-schema` shape verbatim so every fork starts with the same versioned knowledge plane + work-items source-of-truth.
...
- **Subpath exports:**
  - `@cogni/hello-world-doltgres-schema` — root barrel re-exports every slice
  - `@cogni/hello-world-doltgres-schema/work-items` — `work_items` table + `WorkItemRow` / `NewWorkItemRow` inferred types
  - `@cogni/hello-world-doltgres-schema/knowledge` — `knowledge_contributions` + `knowledge_contribution_commits` tables + syntropy seed re-exports from `@cogni/knowledge-base`

## Responsibilities

- **Does:** define Drizzle table schemas for hello-world-local Doltgres tables.
- **Does not:** contain queries, adapters, business logic, RLS policies, or any I/O.

## Dialect separation (non-negotiable)

This package is globbed ONLY by `nodes/hello-world/drizzle.doltgres.config.ts` (Doltgres target). `nodes/hello-world/drizzle.config.ts` (Postgres target) MUST NOT include this path — if it did, the Postgres migrator would try creating Doltgres-only tables in hello-world's Postgres DB.

## Migrator behavior (runs in hello-world migrator initContainer)

```bash
# Container entrypoint for the Doltgres migration:
pnpm db:migrate:hello-world:doltgres:container
```
