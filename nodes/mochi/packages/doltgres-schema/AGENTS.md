# mochi-doltgres-schema · AGENTS.md
...
- **Owners:** @derekg1729
- **Status:** stable
- **Package:** `@cogni/mochi-doltgres-schema`

## Purpose

Pure re-exports of the Drizzle table definitions from `@cogni/knowledge-base`, scoped to the mochi `knowledge_node_template` database. Owned by and namespaced to the mochi node. Mirrors `@cogni/operator-doltgres-schema` shape verbatim so every fork starts with the same versioned knowledge plane + work-items source-of-truth.
...
- **Subpath exports:**
  - `@cogni/mochi-doltgres-schema` — root barrel re-exports every slice
  - `@cogni/mochi-doltgres-schema/work-items` — `work_items` table + `WorkItemRow` / `NewWorkItemRow` inferred types
  - `@cogni/mochi-doltgres-schema/knowledge` — `knowledge_contributions` + `knowledge_contribution_commits` tables + syntropy seed re-exports from `@cogni/knowledge-base`

## Responsibilities

- **Does:** define Drizzle table schemas for mochi-local Doltgres tables.
- **Does not:** contain queries, adapters, business logic, RLS policies, or any I/O.

## Dialect separation (non-negotiable)

This package is globbed ONLY by `nodes/mochi/drizzle.doltgres.config.ts` (Doltgres target). `nodes/mochi/drizzle.config.ts` (Postgres target) MUST NOT include this path — if it did, the Postgres migrator would try creating Doltgres-only tables in mochi's Postgres DB.

## Migrator behavior (runs in mochi migrator initContainer)

```bash
# Container entrypoint for the Doltgres migration:
pnpm db:migrate:mochi:doltgres:container
```
