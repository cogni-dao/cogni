...
- **Container wiring:** `src/bootstrap/container.ts:398-442` — requires ALL THREE Privy env vars AND valid `operator_wallet.address` in repo-spec AND valid `governance.dao_contract`. If any are missing, adapter is `undefined` and settlement silently no-ops (no error)
